...
def run_compare(symbols: list, years: int = 2) -> None:
    """Run all three approaches on the same data and print a comparison table.
    ...
    for symbol in symbols:
        ...
        df_raw = fetcher.fetch_historical(symbol, period=period)
        if df_raw.empty:
            logger.error(f"Failed to fetch data for {symbol}")
            continue
...
def main():
    ...
    if args.mode == "backtest":
        ...
        run_trade(use_ml=use_ml, use_regime=args.regime)
